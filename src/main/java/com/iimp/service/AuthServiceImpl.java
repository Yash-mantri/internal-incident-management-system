...
public class AuthServiceImpl implements AuthService {
	...
	public AuthDtos.LoginResponse login(AuthDtos.LoginRequest req) {
		...
		if (!passwordEncoder.matches(req.getPassword(), user.getPassword())) {
			...
			if (attempts >= MAX_FAILED_ATTEMPTS) {
				...
				throw new LockedException("Account locked after " + MAX_FAILED_ATTEMPTS
						+ " failed attempts. Try again in " + LOCK_MINUTES + " minutes.");
			}
			userRepository.save(user);
		...
		return new AuthDtos.LoginResponse(accessToken, refreshToken, user.getEmail(), user.getName(),
				user.getRole().name(), user.getDepartment(), expirationMs / 1000, user.isFirstLogin());
	}
	...
	public void changePassword(AuthDtos.ChangePasswordRequest req) {
		...
		if (req.getOldPassword().equals(req.getNewPassword())) {
			throw new BadRequestException("New Password is not same as the old password !");
		}
		switch (req.getPurpose()) {

		case NEW_LOGIN:
			otpService.verify(req.getToken(), user.getEmail(), req.getOtpCode(), EventType.NEW_LOGIN);
			break;

		case CHANGE_PASSWORD:
			otpService.verify(req.getToken(), user.getEmail(), req.getOtpCode(), EventType.CHANGE_PASSWORD);
			break;

		default:
			throw new BadRequestException("Only NEW_LOGIN and CHANGE_PASSWORD purpose is valid in that method");
		}
		...
		notificationService.send(user, null, EventType.CHANGE_PASSWORD);
	}
	...
	public AuthDtos.LoginResponse refreshToken(AuthDtos.RefreshTokenRequest req) {
		...
		return new AuthDtos.LoginResponse(newAccess, newRefresh, user.getEmail(), user.getName(), user.getRole().name(),
	...
	public void forgotPassword(@Valid AuthDtos.ForgotPasswordRequest req) {
		...
		otpService.verify(req.getToken(), user.getEmail(), req.getOtpCode(), EventType.FORGOT_PASSWORD);
		if (user.getPassword() != req.getNewPassword()) {
			user.setPassword(passwordEncoder.encode(req.getNewPassword()));
			user.setFirstLogin(false);
			userRepository.save(user);

			notificationService.send(user, null, EventType.FORGOT_PASSWORD);

		} else
			throw new BadRequestException("New Password cannot be same as the Old password");
