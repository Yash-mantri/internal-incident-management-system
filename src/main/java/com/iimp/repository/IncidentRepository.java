...
public interface IncidentRepository extends JpaRepository<Incident, Long> {
    ...
    // Support staff — assigned tickets
    Page<Incident> findByAssignedTo(User user, Pageable pageable);

    // Manager — tickets in their department (by category department)
    ...
    // SLA scheduler: open/in-progress tickets with breached SLA
    @Query("""
        SELECT i FROM Incident i
    ...
    // Critical unassigned for > 1 hour
    @Query("""
        SELECT i FROM Incident i
